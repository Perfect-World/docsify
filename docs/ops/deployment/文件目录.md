...
# 递归创建所有目录
sudo mkdir -p /yykj/{deployment/{docker,docker-images,git-config,mysql-config,nginx-config,sh},backup/{logs,java,vue,h5,mysql},environment/{docker,git/.ssh,node/{node_modules,repositories},maven/maven-jar,nginx,mysql/{conf,data,logs}},project/{scsmp-java/{code,files,logs,jars},scsmp-vue/{code,data,dist,logs,conf},scsmp-h5/{code,dist,logs,conf}}}
# 赋予目录基础读写权限
